#!/usr/bin/env python3
"""Turn a finished report.md into the typeset HTML of Prompt 10.

Vёрстка was the one manual step left in the pipeline, and a manual step in a
pipeline that exists to stop mistakes is where mistakes come back. The script
takes the report and the template that ships with the skill, and applies the
three visual types Prompt 10 requires:
...
Stone blocks are coloured by the three categories of Prompt 08: recommended,
supporting, not without further checking.

    python3 scripts/build-report.py clients/ivan
"""
...
TEMPLATE = (Path(__file__).resolve().parent.parent / "plugins" / "jyotish" /
            "skills" / "jyotish-reading" / "assets" / "11_Шаблон_вёрстки.html")
...
if __name__ == "__main__":
    ...
    pdf = print_pdf(target)
    if pdf is None:
        print("браузера для печати не нашлось; собран только report.html", file=sys.stderr)
        raise SystemExit(1)
    moved = repaginate(target, chapter_pages(pdf, chapter_titles(target)))
    if moved:
        print(f"номера страниц в оглавлении исправлены: {moved}")
        build(target)
        print_pdf(target)
    print(pdf)
